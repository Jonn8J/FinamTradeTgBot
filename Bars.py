...
logger = logging.getLogger('Bars')  # Будем вести лог. Определяем здесь, т.к. возможен внешний вызов ф-ии
datapath = os.path.join(Config.FilePath)  # Путь сохранения файла истории
...
min_bar_open_utc = datetime(2019, 1, 1, tzinfo=timezone.utc)  # Дата, когда никакой тикер еще не торговался
...
def load_candles_from_file(class_code, security_code, tf) -> pd.DataFrame:
    ...
    if os.path.isfile(filename):  # Если файл существует
        ...
        print(f"[DEBUG] Пробуем открыть файл: {filename}")
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    fp_provider = FinamPy(Config.ClientIds, Config.AccessToken)  # Подключаемся ко всем торговым счетам
    ...
    security_codes = Config.portfolio  # Для тестов
